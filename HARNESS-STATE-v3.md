...
**Date:** 2026-05-15  
**Author:** Bryan Sparks  
**Status:** Active development — All phases complete (924 tests passing)
...
Armature synthesizes six papers published between February and May 2026, all converging on the same insight: **the harness is more important than the model.**
...
**Armature contributions:** `HookRegistry` (pre/post-stage and pre/post-tool), `ToolSafetyRule` + `SafetyCondition` declarative YAML DSL (6 operators: `contains`, `not_contains`, `equals`, `not_equals`, `matches_regex`, `truthy`; actions: `block`, `warn`, `log`), `ToolBlocked` non-retryable exception type.
...
Actions: `block` (raises `ToolBlocked`, not retried), `warn` (Python warning, continues), `log` (structured log, continues).
...
All planned phases are complete. 924 tests, passing.
...
| Lifecycle hooks — pre/post stage and tool | ✅ Done | `HookDecision.BLOCK` / `ALLOW` |
| Declarative safety rules (YAML) | ✅ Done | `ToolSafetyRule` + `SafetyCondition`; 6 operators; block/warn/log |
...
| FastAPI HTTP service | ✅ Done | `armature serve` |
...
| Declarative safety rule DSL | `ToolSafetyRule` + `SafetyCondition` | ✅ |
...
**All research gaps are closed.** The next capability horizon is model-tier registration automation (auto-registering a LoRA-fine-tuned Qwen checkpoint as a model tier after export) and a visual workflow editor.
...
Armature is a production-grade agent harness synthesized from five papers spanning February–May 2026. It handles the structural engineering — orchestration, quality control, failure recovery, observability, safety enforcement, and self-improvement — so that every team building on top of it can focus on the domain problem rather than the execution infrastructure.

The fifth paper, Continual Harness (arXiv:2605.09998), completed the picture. Where the prior four papers established the harness architecture, the orchestration model, the synthesis loop, and the safety layer, Continual Harness answered the question the others left open: **how does the harness learn from itself without human intervention?** The answer is two loops — an inner loop (`post_run` refiner stage) that diagnoses each run immediately after it completes, and an outer loop (`SelfImproveRunner`) that accumulates evidence across runs and revises the spec when IHR falls below target.

With all six papers implemented, the flywheel is running. Every run produces training signal. Every improvement proposal now carries a falsifiable contract that is verified on the next cycle. Every high-quality trace is exportable for SLM fine-tuning. The harness compounds — and now it explains itself as it does.

924 tests. All research gaps closed.

---

*For implementation details, see `VISION.md`. For the full test suite, see `tests/`.*
