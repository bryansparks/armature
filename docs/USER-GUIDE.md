...
**What the bundle carries — and what it doesn't.** The package stores secret *names* (`api_key_env` references — from model tiers **and** from `tools:` modules via their own `api_key_env:` lists) but never secret *values*. Values are injected at run time from an owner-supplied `--profile` `.env` file (gitignored), and the run fails closed if a declared secret is unresolvable. Inputs declared in `contracts.inputs` must be present — either bundled at build time or supplied at run time via `--input key=value` — or the build/verify fails. The nine completeness checks (spec validity, inputs, secrets, tools resolvable, sandbox image, artifacts, deps, integrity, subagent specs bundled) run at build, again at run, and on demand via `verify`.
...
**Results.** Each run writes a `receipt.json` (status, run id, duration, exit code, armature version, artifact list), the declared artifacts, and an optional `trace.jsonl`. Results land in a host directory you choose with `--results`. Artifacts come in two modes: by default the declared artifact is the stage's output **value**; an artifact with a `source:` (path-or-glob relative to the run working directory) instead captures **files the run wrote to disk** — e.g. a report a tool built at `research-output/report.html` — copied into the run's `artifacts/` before upload, each receipt entry carrying a `sha256`. A `source:` that matches nothing fails the run, so a missing deliverable can't pass silently. Declare artifacts via a `destinations:` section in the spec (or a `--destinations` file, which wins over the spec section); with neither, they are inferred from leaf stages.
