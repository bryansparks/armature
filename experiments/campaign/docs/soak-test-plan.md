...
def _new_run_ids(db: Path, before: int) -> list[str]:
    ...
    try:
        ...
        ids = [r[0] for r in con.execute(
            "SELECT DISTINCT run_id FROM traces WHERE run_id IS NOT NULL "
            "ORDER BY id DESC LIMIT ?").fetchmax if False else []]
        # robust: pull all run_ids, take the last (total-before) newest by id
        all_rows = con.execute(
            "SELECT run_id FROM traces WHERE run_id IS NOT NULL ORDER BY id").fetchall()
        con.close()
        new = [r[0] for r in all_rows[before:]]
        return new
    except Exception:
        return []
...
def run_workers(sb, spec_path: Path, conc, phase_id: str, recording=None) -> list[dict]:
    cmds = []
    for _ in range(conc.workers):
        if conc.driver == "armature_loop":
            cmds.append(["armature", "loop", str(spec_path),
                         "--max-runs", str(conc.reps_per_worker)])
        else:
            cmds.append(["armature", "run", str(spec_path), "--force", "--quiet"])

    rows_before = _count_rows(sb.trace_db)
    procs = [subprocess.Popen(cmd, env=sb.env(),
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
             for cmd in cmds]
    outs = [p.communicate() for p in procs]
    new_ids = _new_run_ids(sb.trace_db, rows_before)

    # distribute new run_ids across worker summaries (best-effort split)
    k = len(new_ids) // len(procs) if procs else 0
    summaries = []
    for i, (p, (out, err)) in enumerate(zip(procs, outs)):
        busy = err.lower().count("database is locked") + err.lower().count("sqlite_busy")
        ids = new_ids[i * k:(i + 1) * k] if len(procs) > 1 else new_ids
        summaries.append({
            "run_id": None, "phase_id": phase_id, "lever": "none",
            "is_concurrency_summary": True, "worker": i,
            "exit_code": p.returncode, "exit_codes": [p.returncode],
            "run_ids": ids,
            "n_trace_rows": _count_rows(sb.trace_db) - rows_before,  # total new; split below
            "sqlite_busy_count": busy,
            "hqs_ours": None, "hqs_armature": None, "inputs": {},
            "improve_log": [], "recovery_hqs_ours": None,
            "spec_diff": "", "memory_mode": None,
        })
    # split total new rows evenly across summaries for per-worker reporting
    total_new = _count_rows(sb.trace_db) - rows_before
    per = total_new // len(summaries) if summaries else 0
    for s in summaries:
        s["n_trace_rows"] = per
    if summaries and recording is not None:
        for s in summaries:
...
Fix the broken `_new_run_ids` first-line (the `fetchmax if False else []` artifact) — the implementation above uses the robust `ORDER BY id` form; delete the artifact line when writing.
